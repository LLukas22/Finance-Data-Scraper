...
def create_point(timestamp:datetime,row:pd.Series,ticker:Ticker,interval:str,buffer:Buffer,minimal_date:datetime.datetime=datetime.date(1, 1, 1))->bool:
    ...
    buffer.row(
        ...
        columns={
            ...
            "volume": min(sys.maxsize,int(row["Volume"])) 
        },
        at=TimestampNanos(timestamp.value)
    )
    return True

def gather_data(exchange:str,executionContext:ExecutionContext,now:datetime.datetime=datetime.datetime.now()):
    """
...
def download_in_slices(tickers:list[Ticker],interval:str,exchange:str,start:datetime.datetime,stop:datetime.datetime,executionContext:ExecutionContext,include_start:bool=True,slice_size:int=6)->pd.DataFrame:
    """
    ...
    for i,offset in enumerate(offsets[:-1]):
        local_start  = start+datetime.timedelta(days=offset)
        local_end = start+datetime.timedelta(days=offsets[i+1])
        ...
        if data is not None:
            store_points(data,tickers,f"Intraday Tickers (Slice {i+1}/{len(offsets[:-1])})",executionContext,interval,exchange,datetime.date(1, 1, 1) if include_start else start)
        else:
            joined_tickers = ",".join(tickers)
            logging.warning(f"Could not download data for {joined_tickers} from {local_start} to {local_end}")
    
    
def store_points(data:pd.DataFrame,tickers:list[Ticker],message:str,executionContext:ExecutionContext,interval:str,exchange:str,minimal_date:datetime.datetime=datetime.date(1, 1, 1))->None:
...
def flow(exchange:str,interval:str,executionContext:ExecutionContext,now:datetime.datetime):
    """
    ...
    existing_tickers = executionContext.questClient.get_existing_tickers_for_interval(interval)
    ...
    if len(tickers_to_gather) > 0:
        ...
        if interval_type == IntervalTypes.Daily:
            ...
            download_in_slices(tickers_to_gather,interval,exchange,now-datetime.timedelta(days=29),now,executionContext,) 
    ...
    for date in tickers_to_gather:
        ...
        if interval_type == IntervalTypes.Intraday and now-date > datetime.timedelta(days=6):
            if now-date > datetime.timedelta(days=30):
                ...
                download_in_slices(batched_tickers,interval,exchange,now-datetime.timedelta(days=29),now,executionContext,include_start=False)  
            else:
